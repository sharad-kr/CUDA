...
void inference(int num_images, float* images, cudaStream_t streams[],int numStreams){
    ...
    cudaMalloc((void**)&d_layer1_nodes, sizeof(float) * 20 * (24*24)*numStreams);
    cudaMalloc((void**)&d_layer2_nodes, sizeof(float) * 20 * (12*12)*numStreams);
    cudaMalloc((void**)&d_layer3_nodes, sizeof(float) * 50 * (8*8)*numStreams);
    cudaMalloc((void**)&d_layer4_nodes, sizeof(float) * 50 * (4*4)*numStreams);
    cudaMalloc((void**)&d_layer5_nodes, sizeof(float) * 500 * (1*1)*numStreams);
    cudaMalloc((void**)&d_layer6_nodes, sizeof(float) * 10 * (1*1)*numStreams);
...
{
    cudaMemsetAsync(&d_layer1_nodes[image_index%numStreams], 0, 20 * (24*24) * sizeof(float),streams[image_index%numStreams]);
    cudaMemsetAsync(&d_layer2_nodes[image_index%numStreams], 0, 20 * (12*12) * sizeof(float),streams[image_index%numStreams]);
    cudaMemsetAsync(&d_layer3_nodes[image_index%numStreams], 0, 50 * (8*8) * sizeof(float),streams[image_index%numStreams]);
    cudaMemsetAsync(&d_layer4_nodes[image_index%numStreams], 0, 50 * (4*4) * sizeof(float),streams[image_index%numStreams]);
    cudaMemsetAsync(&d_layer5_nodes[image_index%numStreams], 0, 500 * (1*1) * sizeof(float),streams[image_index%numStreams]);
    cudaMemsetAsync(&d_layer6_nodes[image_index%numStreams], 0, 10 * (1*1) * sizeof(float),streams[image_index%numStreams]);

    dim3 blockDim1(20,1,48);
    dim3 gridDim1(1,1,12);
    global_cudaImageToFirstConv<<<gridDim1,blockDim1,0,streams[image_index%numStreams]>>>(1, 20, 28, 5,image_index ,d_images, d_kernel_conv1, &d_layer1_nodes[image_index%numStreams],d_bias_conv1);


    dim3 blockDim2(20,12,4);
    dim3 gridDim2(1,2,6);
    global_cudaMaxPool<<<gridDim2,blockDim2,0,streams[image_index%numStreams]>>>(20, 24,  &d_layer1_nodes[image_index%numStreams],  &d_layer2_nodes[image_index%numStreams]);


    dim3 blockDim3(10,4,8);
    dim3 gridDim3(5,5,8);
    global_cudaConvolution4D<<<gridDim3,blockDim3,0,streams[image_index%numStreams]>>>(20, 50, 12, 5, &d_layer2_nodes[image_index%numStreams], d_kernel_conv2, &d_layer3_nodes[image_index%numStreams],d_bias_conv2);
   
    dim3 blockDim4(50,4,4);
    dim3 gridDim4(1,2,2);
    global_cudaMaxPool<<<gridDim4,blockDim4,0,streams[image_index%numStreams]>>>(50, 8,  &d_layer3_nodes[image_index%numStreams],  &d_layer4_nodes[image_index%numStreams]);

    dim3 blockDim5(25,25,1);
    dim3 gridDim5(21,2,1);
    global_cudaConvolution4D<<<gridDim5,blockDim5,0,streams[image_index%numStreams]>>>(50, 500, 4, 4, &d_layer4_nodes[image_index%numStreams], d_kernel_fc1, &d_layer5_nodes[image_index%numStreams],d_bias_fc1);

    dim3 blockDim6(500,1,1);
    dim3 gridDim6(1,1,1);
    global_cudaReLU<<<gridDim6,blockDim6,0,streams[image_index%numStreams]>>>(500, &d_layer5_nodes[image_index%numStreams]);
   
    dim3 blockDim7(6,101,1);
    dim3 gridDim7(2,5,1);
    global_cudaConvolution4D<<<gridDim7,blockDim7,0,streams[image_index%numStreams]>>>(500, 10, 1, 1, &d_layer5_nodes[image_index%numStreams], d_kernel_fc2, &d_layer6_nodes[image_index%numStreams],d_bias_fc2); 

    dim3 blockDim8(2,1,1);
    dim3 gridDim8(1,1,1);
    global_softmax<<<gridDim8,blockDim8,0,streams[image_index%numStreams]>>>(10,&d_layer6_nodes[image_index%numStreams]); 
    
    

    dim3 blockDim9(10,1,1);
    dim3 gridDim9(1,1,1);
    global_storeAns<<<gridDim9,blockDim9,0,streams[image_index%numStreams]>>>(image_index, d_ans_probabilities, &d_layer6_nodes[image_index%numStreams]);
}
...
void read_folder(const char* folder){
    ...
    int count=0;
    // initialise an array of string type
...
inference(10000,images,streams,numStreams);
